...
static cl::opt<bool>
Flattening("fla",cl::init(false),cl::desc("Enable the flattening pass"));

static cl::opt<bool>
BogusControlFlow("bcf",cl::init(false),cl::desc("Enable bogus control flow"));
...
PassManagerBuilder::PassManagerBuilder() {
    ...
    if(!AesSeed.empty()) {
        llvm::cprng->prng_seed (AesSeed.c_str());
    }
...
void PassManagerBuilder::populateFunctionPassManager(FunctionPassManager &FPM) {
  ...
  // seems to break things for optimization and analysis passes.
  if(Flattening) {
