//===- PrngAESCtr.cpp - AES-based Pseudo-Random Generator ------------------------===//
//
...
namespace llvm {
    ManagedStatic<PrngAESCtr> cprng;
}

const uint32_t AES_RCON[10] = {
    0x01000000UL, 0x02000000UL,
...
PrngAESCtr::PrngAESCtr () {
    seeded = false;
}

unsigned PrngAESCtr::scramble32(const unsigned in, const char key[16])
{
    assert (key != NULL && "PrngAESCtr::scramble key=NULL");
...
void PrngAESCtr::prng_seed (const std::string _seed)
{
    unsigned char s[16];
    unsigned int i = 0;
    
    assert (_seed != NULL && "PrngAESCtr::prng_seed _seed=NULL");
    
    /* We accept a prefix "0x" */
    if ( !(_seed.size() == 32 || _seed.size() == 34) ) {
        LLVMContext &ctx = llvm::getGlobalContext();
        ctx.emitError(Twine ("The AES-CTR PRNG seeding mechanism is expecting a 16-byte value expressed in hexadecimal, like DEAD....BEEF"));
    }
    ...
    // key initial value
    memcpy (key, s, 16);
    DEBUG_WITH_TYPE ("cprng", dbgs() << "CPNRG seeded with " << _seed);
...
PrngAESCtr::~PrngAESCtr() {
    ...
    memset (ctr,  0, 16);
    memset (pool, 0, PRNGAESCTR_POOL_SIZE);
    
    idx = 0;
}

void PrngAESCtr::populate_pool () {

    statsPopulate++;
    
    for (int i = 0; i < PRNGAESCTR_POOL_SIZE; i+=16) {
...
void PrngAESCtr::get_bytes (char *buffer, const int len) {
    
    int sofar = 0, available = 0;
    
    assert (buffer != NULL && "PrngAESCtr::get_bytes buffer=NULL");
    assert (len > 0 && "PrngAESCtr::get_bytes len <= 0");
...
uint8_t PrngAESCtr::get_uint8_t ()
{
    char ret;
...
uint32_t PrngAESCtr::get_uint32_t ()
{
    char tmp[4];
    uint32_t ret = 0;
...
uint64_t PrngAESCtr::get_uint64_t ()
{
    char tmp[8];
    uint64_t ret = 0;
...
uint32_t PrngAESCtr::get_range (const uint32_t max)
{
    uint32_t log, r, mask;
...
void PrngAESCtr::aes_compute_ks (uint32_t *ks, const char *k)
{
    int i;
    uint32_t *p, tmp;
    ...
    while (1) {
        tmp = p[3];
        tmp = ((AES_TE4_3 (BYTE(tmp, 2))) ^
               (AES_TE4_2 (BYTE(tmp, 1))) ^
               (AES_TE4_1 (BYTE(tmp, 0))) ^
               (AES_TE4_0 (BYTE(tmp, 3))));
...
void PrngAESCtr::aes_encrypt (char *out, const char *in, const uint32_t *ks)
{
    ...
    while (1) {
        ...
        tmp0 = AES_TE0 (BYTE (state0, 3)) ^
        AES_TE1 (BYTE (state1, 2)) ^
        AES_TE2 (BYTE (state2, 1)) ^
        AES_TE3 (BYTE (state3, 0)) ^
        ks[r + 0];
        
        tmp1 = AES_TE0 (BYTE (state1, 3)) ^
        AES_TE1 (BYTE (state2, 2)) ^
        AES_TE2 (BYTE (state3, 1)) ^
        AES_TE3 (BYTE (state0, 0)) ^
        ks[r + 1];
        
        tmp2 = AES_TE0 (BYTE (state2, 3)) ^
        AES_TE1 (BYTE (state3, 2)) ^
        AES_TE2 (BYTE (state0, 1)) ^
        AES_TE3 (BYTE (state1, 0)) ^
        ks[r + 2];
        
        tmp3 = AES_TE0 (BYTE (state3, 3)) ^
        AES_TE1 (BYTE (state0, 2)) ^
        AES_TE2 (BYTE (state1, 1)) ^
        AES_TE3 (BYTE (state2, 0)) ^
        ks[r + 3];
    ...
    r +=4;
    state0 = (AES_TE4_3 (BYTE (tmp0, 3))) ^
    (AES_TE4_2 (BYTE (tmp1, 2))) ^
    (AES_TE4_1 (BYTE (tmp2, 1))) ^
    (AES_TE4_0 (BYTE (tmp3, 0))) ^
    ks[r + 0];
    
    state1 = (AES_TE4_3 (BYTE (tmp1, 3))) ^
    (AES_TE4_2 (BYTE (tmp2, 2))) ^
    (AES_TE4_1 (BYTE (tmp3, 1))) ^
    (AES_TE4_0 (BYTE (tmp0, 0))) ^
    ks[r + 1];
    
    state2 = (AES_TE4_3 (BYTE (tmp2, 3))) ^
    (AES_TE4_2 (BYTE (tmp3, 2))) ^
    (AES_TE4_1 (BYTE (tmp0, 1))) ^
    (AES_TE4_0 (BYTE (tmp1, 0))) ^
    ks[r + 2];
    
    state3 = (AES_TE4_3 (BYTE (tmp3, 3))) ^
    (AES_TE4_2 (BYTE (tmp0, 2))) ^
    (AES_TE4_1 (BYTE (tmp1, 1))) ^
    (AES_TE4_0 (BYTE (tmp2, 0))) ^
    ks[r + 3];
